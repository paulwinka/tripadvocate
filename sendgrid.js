...
const sendVerifyEmail = async (user) => {
  ...
  const msg = {
    ...
    dynamicTemplateData: { token: token, url: `https://tripadvocate.herokuapp.com/account/verify_email/${token}` },
    trackingSettings: {
...
const sendResetPassword = async (user) => {
  ...
  const msg = {
    ...
    dynamicTemplateData: { token: token, url: `https://tripadvocate.herokuapp.com/account/reset_password/${token}` },
    trackingSettings: {
      clickTracking: { enable: false },
      openTracking: { enable: false },
      subscriptionTracking: { enable: false },
