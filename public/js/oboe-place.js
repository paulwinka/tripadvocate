        ...
        <div class="col col-md-6 col-lg-4 col-xl-3 rounding-edge">
          <div class="card mb-2 rounding-edge">
            <div class="card-body m-0 p-0 d-flex flex-row justify-content-center">
              <div class="badge ${item.category == `hotel` ? `badge-light` : ``} ${ item.category == `activity` ? `badge-info` : `` }${item.category == `restaurant` ? `badge-success` : ``}">${item.category}</div> 
            </div>
            <a href="/place/${item._id}"><img class="card-img-top" src="/uploads/${ item.image ? item.image.filename : `slam.jpg` }" alt=""></img></a>
